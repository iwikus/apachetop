...
The Debian binary package is intentionally named **apachetop**, even though Debian already has a different package with that name. The Debian package version uses epoch `1:` so this build is not treated as a downgrade purely because it has the same package name. Release assets are named with their target suite, for example `apachetop_0.1.0-2_bookworm_amd64.deb`, `apachetop_0.1.0-2_trixie_amd64.deb`, `apachetop_0.1.0-2_jammy_amd64.deb`, `apachetop_0.1.0-2_noble_amd64.deb`, and `apachetop_0.1.0-2_resolute_amd64.deb`.
